 ...
 */
@Component
public class PackageExpireRemindJob {

    private final static Logger log = LoggerFactory.getLogger(PackageExpireRemindJob.class);
    ...
    private void startHandler(List<MemberPackagePO> list){
        ...
        list.forEach(x -> {
            try{
                ...
                log.error("套餐到期提醒处理失败，主键id:{}", x.getSid());
            }
        });
    }
    ...
    private boolean checkRenew(MemberPackagePO memberPackagePO){
        ...
        countMemberPackageDTO.setStartDtMin(LocalDateTime.now().minusDays(1)
    ...
    private void sendDialogue(MemberPackagePO memberPackagePO){
        ...
        dialoguePO.setMsgType(DialogueConstant.DIALOGUE_PACKAGE_EXPIRE_REMIND_MSG_TYPE);
    ...
    private void sendWechatMessage(MemberPackagePO memberPackagePO){
        ...
        addWechatMessageDTO.setDataType(WechatMessageConstant.DATA_TYPE_PACKAGE_EXPIRE_REMIND);
    ...
    private final static String MESSAGE_TITLE = "服务到期提醒";
}
